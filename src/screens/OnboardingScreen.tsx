...
const INTERESTS = [
  ...
  { label:"Something else", emoji:"✨", color:"#FFE5A5", shadow:"#d4a830" },
];
...
export default function OnboardingScreen({ goTo, onComplete }: Props) {
  const { t } = useLanguage();
  ...
  const toggleCustomInterest = () => {
    if (customInterest.trim()) {
      setInterests((current) => current.includes(customInterest.trim())
        ? current.filter((interest) => interest !== customInterest.trim())
        : current.length < 3 ? [...current, customInterest.trim()] : current);
    }
  };
  ...
  const bubbleText = [
    name ? `Hi ${name}! 👋 I'm Benny!` : `${t("What's your name? 😊")} 🐻`,
    interests.length === 0 ? t("Pick what you love!") : interests.length < 3 ? "Pick more! Or let's go →" : "Amazing picks! 🎉",
    skill ? `Let's learn ${skill}! 🌟` : t("What shall we practice today?"),
  ][step];
    ...
    <div className="w-full h-full relative flex overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex flex-col items-center relative z-10 overflow-y-auto py-6 px-4 lg:px-8">
        ...
        <div className="w-full" style={{ maxWidth:480 }}>
          ...
          <div className="mb-5">
            <h1 className="text-3xl lg:text-4xl font-semibold" style={{ fontFamily:"Fredoka", color:"#2D1B0E" }}>{[t("Tell us about you!"),t("What do you love?"),t("Choose your first skill")][step]}</h1>
          ...
          {step === 0 && (
            <div className="space-y-4 anim-fade-up">
              <div className="rounded-[28px] p-5 shadow-clay-white" style={{ background:"#FFFFFF" }}>
                <label className="block mb-2 font-bold" style={{ fontFamily:"Fredoka", color:"#5C3D2E", fontSize:17 }}>{t("What's your name? 😊")}</label>
              ...
              <div className="rounded-[28px] p-5 shadow-clay-honey" style={{ background:"#FAD054" }}>
                <p className="mb-4 font-bold" style={{ fontFamily:"Fredoka", color:"#2D1B0E", fontSize:17 }}>{t("How old are you? 🎂")}</p>
                ...
                <p className="text-center mt-3 text-sm font-semibold" style={{ fontFamily:"Nunito", color:"rgba(45,27,14,0.6)" }}>{t("years old")}</p>
              </div>
            </div>
          )}
          {step === 1 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 anim-fade-up">
              {INTERESTS.map(({label,emoji,color,shadow})=>{ const sel=label === "Something else" ? Boolean(customInterest.trim() && interests.includes(customInterest.trim())) : interests.includes(label); return <button key={label} onClick={()=>label === "Something else" ? setCustomInterestOpen(true) : toggle(label)} className="btn-press card-3d rounded-[28px] p-5 flex flex-col items-center gap-3" style={{ background:sel?color:"#FFFFFF", boxShadow:sel?`0 8px 0 ${shadow}`:"0 8px 0 #c8c0a8", border:sel?`3px solid ${shadow}`:"3px solid transparent", transform:sel?"translateY(4px)":undefined }}><span style={{ fontSize:48, lineHeight:1 }}>{emoji}</span><span style={{ fontFamily:"Fredoka", color:"#2D1B0E", fontSize:18, fontWeight:600 }}>{label === "Something else" && customInterest.trim() ? customInterest.trim() : t(label)}</span>{sel&&<div className="rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ width:24, height:24, background:"#2D1B0E" }}>✓</div>}</button>; })}
              {customInterestOpen && <div className="col-span-2 sm:col-span-3 rounded-[24px] p-4 anim-fade-up" style={{ background:"rgba(255,255,255,0.8)", border:"2px dashed #d4a830" }}>
                <label className="block mb-2" style={{ fontFamily:"Fredoka", color:"#5C3D2E", fontSize:16 }}>{t("Tell Benny what you like")}</label>
                <div className="flex gap-2">
                  <input type="text" value={customInterest} onChange={(event) => setCustomInterest(event.target.value)} placeholder={t("e.g. cooking, cricket, music...")} className="min-w-0 flex-1 rounded-2xl px-4 py-3 outline-none" style={{ fontFamily:"Nunito", fontSize:15, color:"#2D1B0E", background:"#FFFFFF", border:"2px solid #EDE9DC" }} />
                  <button type="button" onClick={toggleCustomInterest} disabled={!customInterest.trim() || (interests.length >= 3 && !interests.includes(customInterest.trim()))} className="btn-press rounded-2xl px-4 font-bold" style={{ background:"#FAD054", color:"#2D1B0E", boxShadow:"0 4px 0 #c8a020", fontFamily:"Fredoka" }}>{t("Add")}</button>
                </div>
              </div>}
            </div>
          )}
          {step === 2 && (
            <div className="flex flex-col gap-3 anim-fade-up">
              {SKILLS.map(({label,emoji,desc})=>{ const sel=skill===label; return <button key={label} onClick={()=>setSkill(label)} className="btn-press rounded-[24px] px-5 py-4 flex items-center gap-4 text-left transition-all" style={{ background:sel?"#7BC7F0":"#FFFFFF", boxShadow:sel?"0 7px 0 #5aaad0":"0 6px 0 #c8c0a8", border:sel?"3px solid #5aaad0":"3px solid transparent" }}><span style={{ fontSize:36, flexShrink:0 }}>{emoji}</span><div><p style={{ fontFamily:"Fredoka", color:"#2D1B0E", fontSize:19, fontWeight:600 }}>{t(label)}</p><p style={{ fontFamily:"Nunito", color:"#5C3D2E", fontSize:13 }}>{t(desc)}</p></div>{sel&&<span className="ml-auto text-2xl">✅</span>}</button>; })}
            </div>
          )}
          <div className="mt-6 flex flex-col gap-3">
            <button onClick={()=>{ if(step<2) setStep(s=>s+1); else onComplete({name,age,interests,skill}); }} disabled={!canNext} className="btn-press w-full rounded-full py-5 font-semibold" style={{ fontFamily:"Fredoka", background:canNext?"#A4D9A1":"#D8D0BC", color:canNext?"#1A4A18":"#8A8070", boxShadow:canNext?"0 8px 0 #7ab877":"0 5px 0 #b0a890", fontSize:22, cursor:canNext?"pointer":"not-allowed" }}>{step===2?`🎮  ${t("Let's Play!")}`:`${t("Next")}  →`}</button>
            {step>0&&<button onClick={()=>setStep(s=>s-1)} className="text-center text-base font-semibold" style={{ fontFamily:"Nunito", color:"#5C3D2E" }}>← {t("Go back")}</button>}
            {step === 0 && <button onClick={() => goTo("parent")} className="btn-press w-full rounded-full py-3 font-semibold" style={{ fontFamily:"Nunito", background:"rgba(255,255,255,0.72)", color:"#5C3D2E", border:"2px solid rgba(92,61,46,0.18)", fontSize:15 }}>🔒 {t("Parent Login")}</button>}
          </div>
        </div>
        </div>
      </div>
