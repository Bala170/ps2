      ...
      {visible&&(
        <div className="anim-bounce-in relative z-30 mx-4 rounded-[36px] overflow-hidden flex flex-col" style={{ width:"min(480px, calc(100vw - 32px))", background:"#FAF8F0", boxShadow:"0 14px 0 rgba(0,0,0,0.18), 0 28px 80px rgba(0,0,0,0.35)" }}>
          <div className="flex flex-col items-center px-8 pt-8 pb-7" style={{ background:isCorrect?"linear-gradient(135deg, #A4D9A1 0%, #7BC7F0 100%)":"linear-gradient(135deg, #FAD054 0%, #F8A4B8 100%)" }}>
            ...
            <h2 className="text-4xl font-semibold text-center" style={{ fontFamily:"Fredoka", color:"#FFFFFF", textShadow:"0 2px 8px rgba(0,0,0,0.15)" }}>{isCorrect?`${t("Great Job!")} 🎉`:`${t("Let's Learn Together!")} 💡`}</h2>
            <p style={{ fontFamily:"Nunito", color:"rgba(255,255,255,0.9)", fontSize:15, marginTop:4 }}>{isCorrect?`+15 ${t("stars")} · ${t("Empathy Explorer")} ${t("badge")}!`:t("You are doing amazing — every try helps!")}</p>
          </div>
          <div className="px-6 pt-6 pb-7 flex flex-col gap-4">
            ...
            <button onClick={()=>goTo("lesson")} className="btn-press w-full rounded-full py-5 font-semibold" style={{ fontFamily:"Fredoka", background:"#A4D9A1", color:"#1A4A18", boxShadow:"0 8px 0 #7ab877", fontSize:21 }}>🗺️  {t("Next Adventure")}</button>
            <button onClick={()=>goTo("lesson")} className="btn-press w-full rounded-full py-4 font-semibold" style={{ fontFamily:"Fredoka", background:"#7BC7F0", color:"#0a3d5c", boxShadow:"0 6px 0 #5aaad0", fontSize:19 }}>🔄  {t("Try Another Story")}</button>
          </div>
        </div>
      )}
    </div>
